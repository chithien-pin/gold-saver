    ...
    <div className="max-w-2xl">
      <section className="rounded-card bg-card shadow-soft overflow-hidden">
        <div className="flex items-center gap-4 px-6 py-4 border-b border-border">
          ...
          <div>
            <h2 className="text-lg font-semibold text-foreground">Nhập Giao Dịch</h2>
            <p className="text-sm text-muted-foreground">Thêm giao dịch mua hoặc bán vàng vào danh mục</p>
          </div>
        </div>
        <form onSubmit={handleSubmit} className="p-6 md:p-8 space-y-5">
          <div>
            <label className="block text-sm font-medium text-muted-foreground mb-2">
              Loại vàng <span className="text-destructive">*</span>
            </label>
            <select
              value={form.goldType}
              onChange={(e) => handleChange('goldType', e.target.value)}
              className="w-full rounded-xl bg-muted border border-border text-foreground px-4 py-2.5 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition"
            >
            ...
            {errors.goldType && (
              <p className="mt-1 text-sm text-destructive">{errors.goldType}</p>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div>
              <label className="block text-sm font-medium text-muted-foreground mb-2">
                Số lượng (chỉ) <span className="text-destructive">*</span>
              </label>
              <input
                type="number"
                step="0.1"
            ...
            <div>
              <label className="block text-sm font-medium text-muted-foreground mb-2">
                Đơn giá (VNĐ/chỉ) <span className="text-destructive">*</span>
              </label>
              <input
                type="text"
              ...
              />
              {errors.pricePerChi && (
                <p className="mt-1 text-sm text-destructive">{errors.pricePerChi}</p>
              )}
            </div>
          </div>
          ...
          {apiError && (
            <p className="text-sm text-destructive bg-coral-soft px-4 py-2.5 rounded-xl">{apiError}</p>
          )}

          <button
            type="submit"
            disabled={submitting}
            className="w-full sm:w-auto px-8 py-3 rounded-xl bg-primary text-primary-foreground font-semibold hover:bg-primary-dark transition-colors shadow-soft disabled:opacity-60 disabled:cursor-not-allowed"
          >
